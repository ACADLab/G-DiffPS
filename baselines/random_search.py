"""
Baseline: Monte Carlo Random Search (Brute Force)
Randomly samples action vectors uniformly from [0,1]^9 and evaluates via SPICE.
Used as the lower-bound baseline in Experiment 4 comparisons.

Usage:
    python3 baselines/random_search.py --topo Loaded_Line --fc 28.0 --budget 2000 --seed 42
    python3 baselines/random_search.py --all-topos --budget 2000 --seed 42
"""
...
def run_random_search(topo: str, spec: dict, budget: int, seed: int, out_dir: str):
    ...
    print(f"[RS] {topo} | fc={spec['fc_ghz']} GHz | budget={budget} | seed={seed}")
    t0 = time.time()

    for i in range(budget):
        action = rng.uniform(0.0, 1.0, size=9).astype(np.float32)
        params = action_to_params(action, topo, spec)
        fc = spec["fc_ghz"]

        passed_prior = check_physics_priors(topo, params, fc)
        if not passed_prior:
            results.append({"step": i, "reward": -5.0, "passed_prior": False, "spice_calls": spice_calls})
            continue
        ...
        results.append({"step": i, "reward": float(reward), "passed_prior": True, "spice_calls": spice_calls})

        if (i + 1) % 100 == 0:
            elapsed = time.time() - t0
            print(f"  step {i+1:4d}/{budget}  spice={spice_calls:4d}  best={best_reward:.3f}  "
                  f"prior_pass={prior_passes/(i+1)*100:.1f}%  {elapsed:.1f}s")
    ...
    summary = {
        ...
        "seed": seed,
        "budget": budget,
        "spice_calls": spice_calls,
        "prior_pass_rate": prior_passes / budget,
        "best_reward": float(best_reward),
        "best_metrics": best_metrics,
        "best_params": best_params,
        "elapsed_s": elapsed,
    }
    print(f"[RS] Done: best_reward={best_reward:.3f}  spice={spice_calls}  "
          f"prior_pass={prior_passes/budget*100:.1f}%  {elapsed:.1f}s")

    os.makedirs(out_dir, exist_ok=True)
    tag = f"{topo}_fc{spec['fc_ghz']}_seed{seed}"
    with open(os.path.join(out_dir, f"rs_{tag}.json"), "w") as f:
        json.dump(summary, f, indent=2)
    with open(os.path.join(out_dir, f"rs_{tag}_trace.jsonl"), "w") as f:
        for r in results:
...
def main():
    ...
    ap.add_argument("--fc", type=float, default=None, help="Override fc_ghz")
    ap.add_argument("--budget", type=int, default=2000)
    ap.add_argument("--seed", type=int, default=42)
    ap.add_argument("--out-dir", default="results/baselines/random_search")
    args = ap.parse_args()
    ...
    for t in topos:
        ...
        if args.fc is not None:
            spec["fc_ghz"] = args.fc
        r = run_random_search(t, spec, args.budget, args.seed, args.out_dir)
        all_results.append(r)

    print("\n=== RANDOM SEARCH SUMMARY ===")
    print(f"{'Topology':<22}  {'Best Reward':>11}  {'SPICE Calls':>11}  {'Prior Pass%':>11}")
    for r in all_results:
        print(f"{r['topology']:<22}  {r['best_reward']:>11.3f}  {r['spice_calls']:>11d}  "
              f"{r['prior_pass_rate']*100:>10.1f}%")
